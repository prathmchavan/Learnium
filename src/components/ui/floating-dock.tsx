...
import { IconCategory2, IconLayoutNavbarCollapse } from "@tabler/icons-react";
import {
    AnimatePresence,
    MotionValue,
    motion,
        ...
        <div className={cn("relative block md:hidden", className)}>
            <AnimatePresence>
                {open && (
                    <motion.div
                        layoutId="nav"
                        className="absolute left-full mb-2 nset-x-0 flex flex-row gap-2"
                    >
                        {items.map((item, idx) => (
                            <motion.div
                                key={item.title}
            ...
            >
                <IconCategory2 className="h-5 w-5 text-white" />
            </button>
        </div>
    );
};
...
const FloatingDockDesktop = ({
    ...
    return (
        <motion.div
            ...
            className={cn(
                "mx-auto hidden md:flex h-16 gap-4 items-end  rounded-2xl bg-gradient-to-l from-[#da22ff]  to-[#9733ee] px-4 pb-3",
                className
            )}
        >
            {items.map((item) => (
